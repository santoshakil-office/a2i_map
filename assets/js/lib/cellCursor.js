...
.directive("cellCursorTextEditor",['$rootScope', '$timeout', 'urls', 'Auth' , 'DataTunnel', function($rootScope, $timeout, urls, Auth, DataTunnel){ 
  return {
    require:'?ngModel',
    link:function(scope,elem,attrs,modelCtrl){
      console.log(scope.user);
      elem.on("blur",function(){
       var id = DataTunnel.get_data()[0].id;
          var data = DataTunnel.get_data()[0];
            modelCtrl.$commitViewValue();
            console.log((modelCtrl.$modelValue));
            scope.finish(modelCtrl.$modelValue);
            scope.$apply();
            Auth.updateSomething(urls.ORDER_UPDATE+id, data, function(res) {
                    swal("Done", "Change Made Successfully");
                // $scope.order = null;
            },function() {
                swal("Error")
            })
